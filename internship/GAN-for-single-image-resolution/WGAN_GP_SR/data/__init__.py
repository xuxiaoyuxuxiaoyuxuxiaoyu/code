...
class Data:
    def __init__(self, args):
        ...
        if not args.test_only:
            ...
            self.loader_train = DataLoader(trainset, args.batch_size, shuffle=True)
            trainset2 = getattr(module_train, args.data_train)(args, train=False)
            self.loader_train2 = DataLoader(trainset2, 1, shuffle=True)
            evalset = getattr(module_train, args.data_train)(args, train=False)
            self.loader_eval = DataLoader(evalset, 1, shuffle=False)
        else:
